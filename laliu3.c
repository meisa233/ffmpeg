...
// output ..............................
    ...
    int streams_[2];
    streams_[0] = videoindex;
    streams_[1] = audioindex;
    for(i=0; i < valid_streams_nb; i++){
        printf("streams_[i]:%d\n",streams_[i]);

        in_stream = pInFmtContext->streams[streams_[i]];
        ...
        if(in_stream->codecpar->codec_type == AVMEDIA_TYPE_VIDEO) 
        {
            ...
            if (ret < 0) {
			av_log(NULL, AV_LOG_ERROR, "Cannot open video encoder for stream video\n");
			return ret;
		}
        ...
        else if(in_stream->codecpar->codec_type == AVMEDIA_TYPE_AUDIO)
        {
	        ret = avcodec_open2(pInCodecCtx, pInCodec, NULL);
            if (ret < 0) {
			av_log(NULL, AV_LOG_ERROR, "Cannot open audio encoder for stream audio\n");
			return ret;
		}
        ...
        if(in_stream->codecpar->codec_type == AVMEDIA_TYPE_VIDEO){
            ...
            pOutCodecCtx->framerate = pInCodecCtx->framerate;
            fprintf(stderr,"pInCodecCtx->framerate:%d\n", pInCodecCtx->framerate);
            ...
            fprintf(stderr,"pOutCodecCtx->sample_aspect_ratio:%d,%d\n",pOutCodecCtx->sample_aspect_ratio.den,pOutCodecCtx->sample_aspect_ratio.num);
	        pOutCodecCtx->pix_fmt = pInCodecCtx->pix_fmt;
	        pOutCodecCtx->time_base = av_inv_q(pInCodecCtx->framerate);
            fprintf(stderr,"time_base->sample_aspect_ratio:%d,%d\n",pOutCodecCtx->time_base.den,pOutCodecCtx->time_base.num);
        }
        else if(in_stream->codecpar->codec_type == AVMEDIA_TYPE_AUDIO){
            ...
            fprintf(stderr,"audio_time_base->sample_aspect_ratio:%d,%d\n",pOutCodecCtx->time_base.den,pOutCodecCtx->time_base.num);
        }
        ...
        if (ret < 0) {
			av_log(NULL, AV_LOG_ERROR, "Failed to copy encoder parameters to output stream video\n");
			return ret;
		}
    ...
    fprintf(stderr,"before in_packet\n");
